...
public class PhotonVoiceSettings : MonoBehaviour
{
    ...
    /// Disconnect voice client as PUN client disconnects.
    ...
    /// Outgoing audio stream sampling rate (applied per every decoder instance).
    ...
    /// Outgoing audio stream bitrate (applied per every decoder instance).
    public int Bitrate = 30000;               // set in inspector

    /// Enable voice detection (applied per every decoder instance).
    public bool VoiceDetection = false;                 // set in inspector

    /// Voice detection threshold (applied per every decoder instance).
