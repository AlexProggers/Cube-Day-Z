...
public class PhotonVoiceNetwork : MonoBehaviour
{
    ...
    public static bool Connect()
    {
        ...
        {
            string voiceMasterAddress = string.Format("{0}:{1}", PhotonNetwork.PhotonServerSettings.ServerAddress, PhotonNetwork.PhotonServerSettings.VoiceServerPort);
            if (Client.AuthValues == null)
            {
                Client.AuthValues = new LoadBalancing.AuthenticationValues();
            }
            if (string.IsNullOrEmpty(Client.AuthValues.UserId))
            {
                if (PhotonNetwork.AuthValues != null && !string.IsNullOrEmpty(PhotonNetwork.AuthValues.UserId))
                {
                    Client.AuthValues.UserId = PhotonNetwork.AuthValues.UserId;
                }
                else
                {
                    Client.AuthValues.UserId = Guid.NewGuid().ToString();
                }
            }
            return instance.client.Connect(voiceMasterAddress, null, null, null, Client.AuthValues);
        }
