...
public class PhotonTransformView : MonoBehaviour, IPunObservable
{
    ...
    private int sync_step;

	private int _syncStepFactor = 3;
    ...
    void Update()
    {
        if (!SyncTransform || this.m_PhotonView == null || this.m_PhotonView.isMine == true || PhotonNetwork.connected == false || !UseInterpolateSync)
        {
            return;
        }
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
		...
		{
			sync_step++;
			if (sync_step % _syncStepFactor != 0)
			{
				return;
			}
		}
